...
- Perez, Gangnet, and Blake, [Poisson Image Editing](https://legacy.sites.fas.harvard.edu/~cs278/papers/poisson.pdf), 2003. Gradient-domain constraints motivate preserving local gradients while correcting boundary conditions. A full-image Poisson field was implemented and then removed because its dense support changed regions that had no seam evidence.
...
- Kazhdan et al., [Distributed Gradient-Domain Processing of Planar and Spherical Images](https://www.cs.jhu.edu/~misha/MyPapers/ToG10.pdf), 2010, and Bhat et al., [Fourier Analysis of the 2D Screened Poisson Equation for Gradient Domain Problems](https://grail.cs.washington.edu/projects/screenedPoissonEq/), 2008, informed that removed full-image experiment. The production engine deliberately keeps only seam-local waves with source-native midpoint anchors.
...
- `f64` two-sided endpoint profiles which decay to exact zero at tile midpoints;
- adaptive alternating closure for cross-axis residuals at seam intersections;
- no full-image exposure gauge, with exact source-white preservation;
